...
def resample(feature, n_frames):
  feature = at_least_3d(feature)
  return ddsp.core.resample(feature, n_timesteps=n_frames)
...
class F0LoudnessPreprocessor(nn.DictLayer):
  ...
  def call(self, audio, loudness_db, f0_hz,
           **kwargs) -> ['f0_scaled', 'ld_scaled']:
    # Resample features to the frame_rate.
    f0_resampled = resample(f0_hz, self.time_steps)
    ld_resampled = resample(loudness_db, self.time_steps)
...
class F0PowerPreprocessor(F0LoudnessPreprocessor):
  ...
  def call(self, audio, loudness_db, f0_hz,
           **kwargs) -> ['f0_scaled', 'ld_scaled', 'pw_scaled', 'pw_db']:
    ...
    f0_scaled, ld_scaled = super().call(audio, loudness_db, f0_hz, **kwargs)
    return f0_scaled, ld_scaled, pw_scaled, pw_db
